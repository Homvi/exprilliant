  ...
  return (
    <div className="flex justify- md:justify-center flex-col items-center gap-6 w-full my-5 md:min-h-[500px]">
      ...
      <h2 className="text-[#052138a4] text-2xl">
        ...
        <span
          className={
            score == numberOfExpressions ? 'text-[#a39301] animate-pulse duration-75 transition-all' : 'text-[#FF9600]'
          }
        >
          {Math.round((animatedScore / numberOfExpressions) * 100)}%
        </span>
      </h2>
      {score <= 4 && <h3 className="text-[#052138a4] text-xl tracking-widest">{score_page.good_job}</h3>}
